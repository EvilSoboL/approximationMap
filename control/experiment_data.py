...
class ExperimentData:
    def __init__(self):
        ...
        self.co_equation_dict = dict()
        self.o2_equation_dict = dict()
    ...
    def conversion_from_ppm_to_mg_m3(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'CO' in list(self.df.columns):
            ...
            df['NOx'] = ((21 - STANDARD_O2) / (21 - df['O2'])) * 2.05 * df['NOx']
    ...
    def get_rbf_data(self, med_filter: bool = True, non_zero: bool = True) -> tuple[np.array, np.array, np.array]:
        """
    ...
    def get_co_min_equations(self):
        """
        Метод, который сохраняет уравнения аппроксимированных линий минимальных значений топливных карт для CO в атрибут
        co_equation_dict.
        """
        # Получение уравнений в ppm
        for experiment_parameters in self.get_all_available_variations():
            if experiment_parameters[2] == 'CO':
                self.get_experiment_data(*experiment_parameters)
                fuel_axis, additive_axis, approximated_component_surface = self.get_rbf_data()
                ...
                if b_val < 0:
                    self.co_equation_dict[f"{self.fuel_name}_{self.additive_name}"] = f"y = {a_val: .2f}x{b_val: .2f}"
                else:
                    self.co_equation_dict[f"{self.fuel_name}_{self.additive_name}"] = f"y  = {a_val: .2f}x +{b_val: .2f}"
        # Получение уравнений в mg/m3
        for experiment_parameters in self.get_all_available_variations():
            if experiment_parameters[2] == 'CO':
                self.get_experiment_data(*experiment_parameters, convert_to_mg_m3=True)
                fuel_axis, additive_axis, approximated_component_surface = self.get_rbf_data()
                ...
                a_val, b_val = params  # Коэффициенты аппроксимированного линейного уравнения
                if b_val < 0:
                    self.co_equation_dict[f"{self.fuel_name}_{self.additive_name}_mg_m3"] = f"y = {a_val: .2f}x{b_val: .2f}"
                else:
                    self.co_equation_dict[f"{self.fuel_name}_{self.additive_name}_mg_m3"] = f"y  = {a_val: .2f}x +{b_val: .2f}"

    def get_o2_min_equations(self):
        """
        Метод, который сохраняет уравнения аппроксимированных линий минимальных значений топливных карт для CO в атрибут
        o2_equation_dict.
        """
        for experiment_parameters in self.get_all_available_variations():
            if experiment_parameters[2] == 'O2':
                ...
                contours = plt.contour(fuel_axis, additive_axis, approximated_component_surface, colors='none',
                                       levels=levels)
    ...
    def get_approx_surface_in_tableview(self):
        ...
        """
        fuel_axis, additive_axis, approximated_surface = self.get_rbf_data()
        df = self.convert_approximated_surface_to_df(fuel_axis, additive_axis, approximated_surface)
        return df
