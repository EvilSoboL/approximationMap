...
class DatabaseHandler:
    ...
    def delete_zeroes_from_experiment_database(self):
        """
        Метод, удаляющий нули в экспериментальных данных, которые использовались для визуального отображения 'коридора'
        и не должны быть в экспериментальных данных.
        """
        with self.connection:
            for component_with_zeroes in ['CO', 'NO2', 'NOx']:
                self.cursor.execute(
                    f"""
